...
class Watchlist(commands.Cog):
    ...
    async def view_watchlist(self, ctx):
        ...
        await ctx.send(f"⏳ Fetching data for {len(stocks)} stocks...")

        embed = discord.Embed(
            title=f"📊 {ctx.guild.name} Watchlist",
            description=f"Tracking {len(stocks)} stock(s)",
            color=config.BOT_COLOR
        )

        for stock in stocks[:10]:  # Limit to 10 stocks to avoid rate limits
            symbol = stock['symbol']
            stock_info = await stock_api.get_stock_info(symbol)
        ...
        if len(stocks) > 10:
            embed.set_footer(text=f"Showing 10 of {len(stocks)} stocks")
        else:
            embed.set_footer(text=f"Use {config.COMMAND_PREFIX}addstock to add more stocks")
